...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only){
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS){
		...
		check(rc == 0, "git failed%c", '.');
	}
	else if(apr_fnmatch(DEPEND_PAT, info.path, 0) == APR_SUCCESS){
		check(!fetch_only, "No Point in fetching a DEPENDS file%c", '.');
...
int Command_build(apr_pool_t *p, const char *url, 
	...
	if(access(CONFIG_SCRIPT, X_OK) == 0) {
		log_info("Has a configure script, running it%c", '.');
		rc = Shell_exec(CONFIGURE_SH, "OPTS", configure_opts, NULL);
		check(rc == 0, "Failed to configure%c", '.')
	}
	
	rc = Shell_exec(MAKE_SH, "OPTS", make_opts, NULL);
	check(rc == 0, "Failed to build%c", '.');
	
	rc = Shell_exec(INSTALL_SH, 
		"TARGET", install_opts ? install_opts : "install",
		NULL);
	check(rc == 0, "Failed to install%c", '.');
	
	rc = Shell_exec(CLEANUP_SH, NULL);
	check(rc == 0, "Failed to cleanup after build%c", '.');
	
	rc = DB_update(url);
	check(rc == 0, "Failed to add this package to database%c", '.');
...
int Command_install(apr_pool_t *p, const char *url, 
		...
		const char *install_opts){
	int rc = 0;
	check(Shell_exec(CLEANUP_SH, NULL) == 0, "Failed to cleanup before building%c", '.');
	
	rc = DB_find(url);
	check(rc != -1, "Error checking the install database%c", '.');

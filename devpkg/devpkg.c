...
int main(int argc, const char const *argv[]){
	...
	switch(request) {
		case COMMAND_INSTALL:
			check(url, "You must at least give a URL%c", '.');
		...
		case COMMAND_LIST:
			...
			break;
		case COMMAND_FETCH:
			check(url != NULL, "You must give an URL%c", '.');
			Command_fetch(p, url, 1);
			log_info("Downloaded to %s and in /tmp/", BUILD_DIR);
			break;
		case COMMAND_BUILD:
			check(url, "You must at least give an URL%c", '.');
			Command_build(p, url, config_opts, make_opts, install_opts);
			break;
		case COMMAND_INIT:
			rv = DB_init();
			check(rv == 0, "Failed to make the database%c", '.');
			break;
		default:
			sentinel("Invalid command given%c", '.');
	}
	//Shell_exec(CLEANUP_SH, NULL); // DEBUG
	//Shell_exec(TAR_SH, "FILE", TAR_BZ2_SRC, NULL); // DEBUG

...
int DB_update(const char *url){
	...
	bconchar(line, '\n'); // Concatenate the character to the end of bstring
	int rc = fwrite(line->data, blength(line), 1, db);
	check(rc == 1, "Failed to append to the db%c", '.');
...
int DB_init() {
	...
	if(access(DB_DIR, W_OK | X_OK) == -1) {
		apr_status_t rc = apr_dir_make_recursive(DB_DIR, 
			...
			APR_GREAD | APR_GWRITE | APR_GEXECUTE, p); // permissions by User and Group
